...
const Token = () => {
    return (
        <main id="token" className="lg:max-h-[calc(100vh-100px)] h-screen">
            <SectionHeader
            ...
            />
            <section className="grid lg:grid-cols-2 gap-14 items-center mt-5">
                {/* Left: Image */}
                <div className="relative flex justify-center items-center">
                    <div className="absolute size-100 bg-gradient-to-br from-[#0092ff] to-[#0029a5] opacity-80 rounded-full blur-3xl -z-10" />
                    <img src={tokenImg} alt="MineVers Token" className="md:size-120 size-90 z-10" loading="lazy" />
                </div>
                    ...
                    <div className="grid md:grid-cols-2 grid-cols-1 gap-15">
                        {tokenFeatures.map((feature, idx) => (
                            <div key={idx} className="flex items-start space-x-4">
                                <feature.Icon className="text-violet-500 text-xl size-8" />
                                <div>
